...
asvTable.BAC.fp <- paste0(input.fp, "/ASVtable_16S_wTax_bonney.txt") 
...
asvTab.BAC.b.b.F <- asvTab.BAC.b.b.1 %>% select(-c(BN_Bulk_X, BN_Bulk_Y, BN_Bulk_Z))

# NP sample that was over-watered at the start of the experiment
asvTab.BAC.b.np.F <- asvTab.BAC.b.np.1 %>% select(-BNM_11)  
...
########################################################
# create row sum column
asvTab.BAC.b.np.F.1 <- asvTab.BAC.b.np.F %>% mutate(total = rowSums(.))

# re-order with greatest row sum 1st
asvTab.BAC.b.np.F.2 <- asvTab.BAC.b.np.F.1 %>% arrange(desc(total))
...
########################################################
# create row sum column
asvTab.BAC.b.np.F.3 <- asvTab.BAC.b.np.F.2 %>% filter(total!= 0)
...
########################################################
# calculate relative abundance of each cell by column value
asvTab.BAC.b.np.F.4 <- asvTab.BAC.b.np.F.3 %>% apply(2, function(x){x/sum(x)})
# multiply by 100 for percentage
asvTab.BAC.b.np.F.5 <- asvTab.BAC.b.np.F.4 %>% apply(2, function(x){x *100})
# classify as a data frame
asvTab.BAC.b.np.F.5 <- as.data.frame(asvTab.BAC.b.np.F.5)
...
########################################################
# move rownames of ASV table into 1st column as 'ASV_ID'
asvTab.BAC.b.np.F.5 <- tibble::rownames_to_column(asvTab.BAC.b.np.F.5, "ASV_ID")
# Left Join using inner_join function from dplyr
asvTab.BAC.b.np.F.T <- asvTab.BAC.b.np.F.5 %>% inner_join(taxTab, by= "ASV_ID")
...
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.b.F.T, paste0(bonney1.fp, "/asvTab.BAC.b.b.F.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.b.F.T, file = paste0(bonney2.fp, "/BonneyBulk_16S_RelativeAbundance.txt"), 
            sep = "\t", row.names = TRUE, col.names = NA)

# CTRL samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.c.5.T, paste0(bonney1.fp, "/asvTab.BAC.b.c.5.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.c.5.T, file = paste0(bonney2.fp, "/BonneyCTRL_16S_RelativeAbundance.txt"), 
            sep = "\t", row.names = TRUE, col.names = NA)

# N samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.n.5.T, paste0(bonney1.fp, "/asvTab.BAC.b.n.5.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.n.5.T, file = paste0(bonney2.fp, "/BonneyN_16S_RelativeAbundance.txt"), 
            sep = "\t", row.names = TRUE, col.names = NA)

# P samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.p.5.T, paste0(bonney1.fp, "/asvTab.BAC.b.p.5.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.p.5.T, file = paste0(bonney2.fp, "/BonneyP_16S_RelativeAbundance.txt"), 
            sep = "\t", row.names = TRUE, col.names = NA)

# NP samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.np.F.T, paste0(bonney1.fp, "/asvTab.BAC.b.np.F.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.np.F.T, file = paste0(bonney2.fp, "/BonneyNP_16S_RelativeAbundance.txt"), 
            sep = "\t", row.names = TRUE, col.names = NA)
```
