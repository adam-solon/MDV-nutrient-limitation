...
subtitle: "  \n Fryxell Basin Surface Cover Over Time"
author: "Adam J. Solon"
date: "`r Sys.Date()`"
...
# Script Summary  
This report plots surface growth of primary producers from Fryxell Basin soils (coastal basin) and uses statistical models to determine if there were differences in surface cover among treatments.  There are four treatment groups from each basin: CTRL (controls with H2O only), N (nitrogen + H2O only), P (phosphorus + H2O only), and NP (N and P combined + H2O). 
...
5.  Format File
6.  Transform data
7.  Plot surface cover time series
8.  Plot maximum surface cover
9.  Statistics
...
### Input file 
1. Count data from Fryxell soil microcosms, filename 'Fryxell_counts_percent.txt' an output from script 'MCosms_II_count_processing.Rmd'  
  
```{r input data files, echo = FALSE, include = FALSE}
# input data files
# Count data from point-intercept method (microscopy)
counts.fp <- paste0(input2.fp, "/Fryxell_counts_percent.txt") 
...
gc.plot.b <- ggplot(df.1, aes(x = days.f, y = primary_cover,
  ...
  scale_x_continuous(expand = c(0.1, 1), limits = c(28, 63), breaks = c(28, 46, 63))+
...
#save as R file
saveRDS(gc.plot.b, paste0(objects2.f.fp, "/FXM_surface_cover_plot.rds"))

```
  
``` {r echo = FALSE, fig.pos = 'h', fig.cap = "Plot of surface cover (%) by treatment for Fryxell basin soils.  Counts begin on day 28 and end on day 63 and each dashed line represents a microcosm."}
#display graph
plot(gc.plot.b)
```
...
#save as R file
saveRDS(df.2, paste0(objects2.f.fp, "/FXM_max_cover.Rds"))

#save as R file
saveRDS(df.2, paste0(input3.fp, "/FXM_max_cover.Rds"))

#save as .txt file 
write.table(df.2, file = paste0(output2.f.fp, "/FXM_max_cover.txt"), 
...
# df.2 <- readRDS(outputs.b.fp, "/FXM_max_cover.Rds")
...
```{r echo = FALSE, fig.pos = 'h', fig.cap = "Plot of average maximum surface cover (%) by treatment for Fryxell basin soils."}
#display
plot(max.box.b)
```
...
#Save as jpg
jpeg(filename = "output/fryxell/max_surface_cover_boxplot.jpeg", 
     width = 800, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
```{r echo = FALSE, fig.pos = 'h', fig.cap = "Plot of maximum surface cover for each microcosm grouped by treatment for Fryxell basin soils."}
#display
plot(max.plot.b)
```
...
```
The likelihood ratio test supports using a model with 'treatment' as a variable compared to a null.  
...
AIC analysis suggests no improvement in model fit w/ beta regression relative to a basic linear model (i.e. ANOVA) or a null model.   
  
\newpage  
  
### Beta regression model  
Current beta regression model has fixed precision parameter.  Build model with a precision parameter modeled from explanatory variables (i.e., a variable dispersion beta regression) to see if a beter fit is acquired and compare with beta regression and linear models.  
...
plot(residuals(l.mod) ~ fitted(l.mod))
...
```{r echo = FALSE, include = FALSE}
#create data frame w/ AICc results
aicC.b.df <- data.frame (Model  = c("beta.regression", "var.disp.beta.reg", "beta.null", "linear.model"),
                  AICc = c(aicC.b, aicC.b.pp, aicC.n, aicC.l),
                  parameters = c(aicC.b.p, aicC.b.pp.p, aicC.n.p, aicC.l.p)
                  )
```
  
```{r kable 3}
#create table of AICc values
knitr::kable(aicC.b.df, booktabs = T, digits = 2, align = "cccc", caption = 'Model Comparison with  second-order Akaike Information Criterion (AICc), a correction for small sample size') %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 14) %>%
  row_spec(0, bold = T)
...
```{r echo = FALSE, include = FALSE}
#create data frame w/ AICc results
aicC.b.bc.df <- data.frame (Model  = c("beta.regression", "var.disp.beta.reg", "beta.null", "linear.model"),
                  AICc = c(aicC.b.bc, aicC.b.pp.bc, aicC.n.bc, aicC.l),
                  parameters = c(aicC.b.bc.p, aicC.b.pp.bc.p, aicC.n.bc.p, aicC.l.p)
                  )
```
  
```{r kable 4}
#create table of AICc values
knitr::kable(aicC.b.bc.df, booktabs = T, digits = 2, align = "cccc", caption = 'Bias Correction Model Comparison with  second-order Akaike Information Criterion (AICc), a correction for small sample size') %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 14) %>%
  row_spec(0, bold = T)
...
```{r echo = FALSE}
#create data frame w/ AICc results
aicC.b.br.df <- data.frame (Model  = c("beta.regression", "var.disp.beta.reg", "beta.null", "linear.model"),
                  AICc = c(aicC.b.br, aicC.b.pp.br, aicC.n.br, aicC.l),
                  parameters = c(aicC.b.br.p, aicC.b.pp.br.p, aicC.n.br.p, aicC.l.p)
                  )
```
  
```{r kable 5}
#create table of AICc values
knitr::kable(aicC.b.br.df, booktabs = T, digits = 2, align = "cccc", caption = 'Bias Reduction Model Comparison with  second-order Akaike Information Criterion (AICc), a correction for small sample size') %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 14) %>%
  row_spec(0, bold = T)
...
### Model Summaries of variable dispersion beta regression with bias correction or reduction, or non-corrected model
```{r echo = FALSE}
...
Final model selection using the principle of parsimony is the basic linear model. There was increased surface cover of P and NP treatments as compared to CTRL.  Follow with a Post Hoc test for pairwise comparison of fitted model using estimated marginal means.  

```{r echo=FALSE}
#save as .txt file
capture.output(summary(l.mod), file = paste0(output2.f.fp, '/FXM_linear_model_summary.txt'))
```

```{r echo = FALSE}
#post-hoc tests to identify which pairwise contrasts of treatments levels are significant
p.hoc <- pairs(emmeans(l.mod, ~ treatment))

#display
print(p.hoc)

#save as .csv file
write.csv(p.hoc, file = paste0(output2.f.fp, '/FXM_lm_pairwise_test.csv'))
``` 
...
```{r echo = FALSE}
#model with explanatory variables = factor 1 (N), factor 2 (P) for interaction effects
l.mod.f <- lm(primary_cover_2 ~ N * P, data = df.2)
...
```{r echo = FALSE}
#Residual plots of b.reg and b.reg.pp
par(mfrow = c(3, 1), oma = c(0, 0, 0, 0), mar = c(4, 4, 0.2, 0.2))
plot(residuals(b.reg.f) ~ fitted(b.reg.f))
plot(residuals(b.reg.f.pp) ~ fitted(b.reg.f.pp))
plot(residuals(l.mod.f) ~ fitted(l.mod.f))
```
...
#parameters in model
aicC.b.pp.f.p <- AICc(b.reg.f.pp, return.K = TRUE, second.ord = TRUE)
...
```{r echo = FALSE, include = FALSE}
#second order AICc (small sample size)
aicC.l.f <- AICc(l.mod.f, return.K = FALSE, second.ord = TRUE) 

#display
print(aicC.l.f)

#parameters in model
aicC.l.f.p <- AICc(l.mod.f, return.K = TRUE, second.ord = TRUE)
...
```{r echo = FALSE, include = FALSE}
#create data frame w/ AICc results
aicC.b.df.f <- data.frame (Model  = c("beta.regression", "var.disp.beta.reg", "linear.model"),
                  AICc = c(aicC.b.f, aicC.b.pp.f, aicC.l.f),
                  parameters = c(aicC.b.f.p, aicC.b.pp.f.p, aicC.l.f.p)
                  )
```
  
```{r kable 6}
#create table of AICc values
knitr::kable(aicC.b.df.f, booktabs = T, digits = 2, align = "ccc", caption = 'Model Comparison with  second-order Akaike Information Criterion (AICc), a correction for small sample size') %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 14) %>%
  row_spec(0, bold = T)

```
  
Model selection using the principle of parsimony provides evidence there was an interaction effect between the N and P treatments. Follow with a Post Hoc test for pairwise comparison of fitted model.  

```{r echo=FALSE}
#save as .txt file
capture.output(summary(l.mod.f), file = paste0(output2.f.fp, '/FXM_linear_model_interaction_summary.txt'))
```
  
```{r echo = FALSE}
#post-hoc tests to identify which pairwise contrasts of treatments levels are significant
p.hoc.f <- test(pairs(emmeans(l.mod.f, ~ N * P, mode = "link")))

#display
print(p.hoc.f)

#save as .csv file
write.csv(p.hoc.f, file = paste0(output2.f.fp, '/FXM_lm_interaction_pairwise_test.csv'))
``` 
  
### Plot  
  
```{r echo = FALSE, fig.pos = 'h', fig.cap = "Interaction plot between N and P factors for Fryxell basin soils."}
#display interaction
emmip(b.reg.f, P ~ N)
```
