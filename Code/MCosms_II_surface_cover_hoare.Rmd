...
subtitle: "  \n Hoare Basin Surface Cover Over Time"
author: "Adam J. Solon"
date: "`r Sys.Date()`"
...
# Script Summary  
This report plots surface growth of primary producers from Hoare Basin soils (intermediate basin) and uses statistical models to determine if there were differences in surface cover among treatments.  There are four treatment groups from each basin: CTRL (controls with H2O only), N (nitrogen + H2O only), P (phosphorus + H2O only), and NP (N and P combined + H2O). 
...
### Input file 
1. Count data from Hoare soil microcosms, filename 'Hoare_counts_percent.txt' an output from script 'MCosms_II_count_processing.Rmd'

```{r input data files, echo = FALSE, include = FALSE}
# input data files
# Count data from point-intercept method (microscopy)
counts.fp <- paste0(input2.fp, "/Hoare_counts_percent.txt") 
...
gc.plot.b <- ggplot(df.1, aes(x = days.h, y = primary_cover,
...
#save as R file
saveRDS(gc.plot.b, paste0(objects2.h.fp, "/HRM_surface_cover_plot.rds"))

```

``` {r echo = FALSE, fig.pos = 'h', fig.cap = "Plot of surface cover (%) by treatment for Hoare basin soils.  Counts begin on day 28 and end on day 63 and each dashed line represents a microcosm."}
#display graph
plot(gc.plot.b)
```
...
#save as R file
saveRDS(df.2, paste0(objects2.h.fp, "/HRM_max_cover.Rds"))

#save as R file
saveRDS(df.2, paste0(input3.fp, "/HRM_max_cover.Rds"))

#save as .txt file 
write.table(df.2, file = paste0(output2.h.fp, "/HRM_max_cover.txt"), 
...
# df.2 <- readRDS(outputs.b.fp, "/HRM_max_cover.Rds")
...
```{r echo = FALSE, fig.pos = 'h', fig.cap = "Plot of average maximum surface cover (%) by treatment for Hoare basin soils."}
#display
plot(max.box.b)
```
...
#Save as jpg
jpeg(filename = "output/hoare/max_surface_cover_boxplot.jpeg", 
     width = 800, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
```{r echo = FALSE, fig.pos = 'h', fig.cap = "Plot of maximum surface cover for each microcosm grouped by treatment for Hoare basin soils."}
#display
plot(max.plot.b)
```
...
# beta regression cab not accept values exact values of 0 or 1 as the response variable.
#Following recommendations from Cribari-Neto and Zeileis (2010), transform data with following function: 
transform01 <- function(x) {
  (x * (length(x) - 1) + 0.5) / (length(x))
}
#
df.2$primary_cover_2_scaled <- transform01(df.2$primary_cover_2)
#
```
...
b.null <- betareg(primary_cover_2_scaled ~ 1, data = df.2)
...
b.reg <- betareg(primary_cover_2_scaled ~ treatment, data = df.2)
...
```{r echo = FALSE}
#linear model
l.mod <- lm(primary_cover_2_scaled ~ treatment, data = df.2)
...
b.reg.pp <- betareg(primary_cover_2_scaled ~ treatment | treatment, data = df.2)
#
summary(b.reg.pp)
```
...
```{r echo = FALSE}
#likelihood-ratio tests to compare variable dispersion model with fixed-Ï and null models.
lrt.2 <- lrtest(b.null, b.reg, b.reg.pp)
...
#bias correction
b.null.bc <- betareg(primary_cover_2_scaled ~ 1, data = df.2, type = "BC")
b.reg.bc <- betareg(primary_cover_2_scaled ~ treatment, data = df.2, type = "BC")
b.reg.pp.bc <- update(b.reg.bc, . ~ . | treatment, type = "BC")

#bias reduction
b.null.br <- betareg(primary_cover_2_scaled ~ 1, data = df.2, type = "BR")
b.reg.br <- betareg(primary_cover_2_scaled ~ treatment, data = df.2, type = "BR")
...
Final model selection using the principle of parsimony is the beta regression without correction. There was increased surface cover of N and NP treatments as compared to CTRL. Follow with a Post Hoc test for pairwise comparison of fitted model using estimated marginal means.   

```{r echo=FALSE}
#save as .txt file
capture.output(summary(b.reg), file = paste0(output2.h.fp, '/HRM_varDisp_beta_regression_summary.txt'))
```
...
#save as .csv file
write.csv(p.hoc, file = paste0(output2.h.fp, '/HRM_varDisp_betaReg_pairwise_test.csv'))
```
...
b.reg.f <- betareg(primary_cover_2_scaled ~ N * P, data = df.2)
...
```{r echo = FALSE}
#model with explanatory variables = factor 1 (N), factor 2 (P) for interaction effects
b.reg.f.pp <- betareg(primary_cover_2_scaled ~ N * P | N * P, data = df.2)
...
```{r echo=FALSE}
#save as .txt file
capture.output(summary(b.reg.f), file = paste0(output2.h.fp, '/HRM_varDisp_beta_regression_interaction_summary.txt'))
```
...
#save as .csv file
write.csv(p.hoc.f, file = paste0(output2.h.fp, '/HRM_varDisp_betaReg_interaction_pairwise_test.csv'))
```
 
```{r echo = FALSE, fig.pos = 'h', fig.cap = "Interaction plot between N and P factors for Hoare basin soils."}
#display interaction
emmip(b.reg.f, P ~ N)
```
