...
subtitle: "  \n Logistic Growth Model - Hoare Basin Soils"
author: "Adam J. Solon"
date: "`r Sys.Date()`"
...
## Run analyses
### Compute percent cover for primary producers - Hoare

```{r hoare format, echo = FALSE, include = FALSE}
#add up prijmary producer counts across columns for 'primary' surface count
df.h.1 <- df.h %>% mutate(primary = green_sphere + 
                          green_dot +
                          green_branching +
                          brown_dot + 
                          green_rope + 
...
df.h.1 <- df.h.1 %>% mutate(primary_cover = (primary / num_counts) * 100,
                            .after = primary)

#compute percent primary producer cover between 0 and 1
df.h.1 <- df.h.1 %>% mutate(primary_cover_2 = (primary / num_counts),
                            .after = primary_cover)

#replace NaN with zeroes
df.h.1 <- df.h.1 %>% mutate_all(~replace(., is.nan(.), 0))

#create vector with days for each plate by date column
days.h <- rep(c("0", "7", "14", "21", "28", "35", "42", "49", "56", "63"), each = 20)

#join vector to data frame
df.h.1 <- cbind(df.h.1, days.h)
...
df.h.1$primary_cover[df.h.1$primary_cover == 0] <- 0.001
...
L <- all_easylinear(primary_cover ~ days.h | plate_number + treatment + replicate, 
                    data = df.h.1, 
                    h = 4, 
                    quota = 0.95)
...
#save as .csv file
write.csv(results, file = paste0(output4.h.fp, '/HRM_easylinear.csv'))
...
xyplot(mumax ~ replicate | treatment, data = results,
       ...
       main="Maximum Specific Growth Rate by Treatment- Hoare Basin")
...
xyplot(lag ~ replicate | treatment, data = results,
       ...
       main="Lag Time by Treatment- Hoare Basin")
...
write.csv(easyfit_model_mean_se.1, file = paste0(output4.h.fp, '/HRM_easylinear_summary_parameters.csv'))
...
p <- c(y0 = 1e-1, mumax = 0.36 , K = 100)

#set a lower value to avoid output of negative parameters
lower = c(y0 = 0, mumax = 0, K = 0)

#logistic model
many_logistic <- all_growthmodels(primary_cover ~ grow_logistic(days.h, parms) | plate_number + treatment,
                                  data = df.h.1,
                                  p = p,
                                  lower = lower, 
                                  ncores = 1)
...
#save as .csv file
write.csv(many_logistic_res, file = paste0(output4.h.fp, '/HRM_logistic_model.csv'))
```
...
write.csv(logistic_model_mean_se.1, file = paste0(output4.h.fp, '/HRM_logistic_model_summary_parameters.csv'))
...
#subset only treatments with useable growth models
many_logistic_res3 <- many_logistic_res2[many_logistic_res2$treatment == "NP",]

#save as R file
saveRDS(many_logistic_res3, paste0(objects4.h.fp, "/HRM_grow_mod_params.rds"))
```

```
No comparison is possible with only the NP treatment yielding a useable model.  
