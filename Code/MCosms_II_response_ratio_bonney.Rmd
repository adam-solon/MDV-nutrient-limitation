...
### Input file 
1. Maxmimum surface growth data from Bonney soil microcosms, filename '/BNM_max_cover.Rds' an output from script 'MCosms_II_surface_cover_bonney.Rmd'  

```{r input data files, echo = FALSE, include = FALSE}
# input data files
# Count data from point-intercept method (microscopy)
counts.fp <- paste0(input3.fp, "/BNM_max_cover.Rds") 
...
#save as csv file
write.csv(dat.all, paste0(output3.b.fp, '/BNM_summary_statistics.csv'))
...
#Save as R file
saveRDS(df.all, paste0(objects3.b.fp, "/BNM_df_all.rds"))

##save as .csv file
write.csv(df.all, file = paste0(output3.b.fp, '/BNM_summary_log_values.csv'))
...
capture.output(summary(lm.LRR), file = paste0(output3.b.fp, "/BNM_lmLRR_summary.txt"))

#save pairwise test of fitted model (estimated marginal means) as .txt file
capture.output(pairs(emmeans(lm.LRR, ~ treatment)), file = paste0(output3.b.fp, "/BNM_lmLRR_pairwise.txt"))
...
#save as .csv file
write.csv(df.LRR, file = paste0(output3.b.fp, '/BNM_dfLRR_summary_statistics.csv'))
```
...
#save as R file
saveRDS(df.LRR, paste0(objects3.b.fp, "/BNM_dfLRR.rds"))
...
```{r echo = FALSE, include = FALSE}
#Restore dfLRR
#df.LRR <- readRDS(objects3.b.fp, "BNM_dfLRR.rds")
...
i.plot.b <- df.LRR %>% 
  ...
  geom_hline(yintercept = 2.026, linetype="dashed", color = "blue", size = 1.2) +
...
``` {r echo = FALSE, fig.pos = 'h', fig.cap = "Interaction plot between N and P of response ratios of nutrient enrichments vs. controls in Bonney basin soils. Red dashed line represents control and blue dashed line indicates an additive."}
#display graph
plot(i.plot.b)
```
...
#Save as jpg
jpeg(filename = "output/bonney/BNM_response_ratio_interaction_plot.jpeg", 
     width = 800, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
i.plot.b.m <- df.LRR %>% 
  ...
  theme(plot.title = element_text(size = 40), 
        ...
        plot.margin = margin(l = 0, # Left margin
                             r = 1,
                             b = 5,  # bottom margin 37, 5
                             t = 10)) 

#save as R file
saveRDS(i.plot.b.m, paste0(objects3.b.fp, "/BNM_lnRR_plot_figure3.rds"))
```
  
## Is there single limitation or co-limitation?  If co-limitation, what type (e.g. super-additive, additive, or sub-additive)? 

The results suggest single P limitation.
