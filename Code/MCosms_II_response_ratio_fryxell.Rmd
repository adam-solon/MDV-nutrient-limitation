...
### Input file 
1. Maxmimum surface growth data from Fryxell soil microcosms, filename '/FXM_max_cover.Rds' an output from script 'MCosms_II_surface_cover_fryxell.Rmd'  

```{r input data files, echo = FALSE, include = FALSE}
# input data files
# Count data from point-intercept method (microscopy)
counts.fp <- paste0(input3.fp, "/FXM_max_cover.Rds") 
...
#save as csv file
write.csv(dat.all, paste0(output3.f.fp, '/FXM_summary_statistics.csv'))
...
#Save as R file
saveRDS(df.all, paste0(objects3.f.fp, "/FXM_df_all.rds"))

##save as .csv file
write.csv(df.all, file = paste0(output3.f.fp, '/FXM_summary_log_values.csv'))
...
capture.output(summary(lm.LRR), file = paste0(output3.f.fp, "/FXM_lmLRR_summary.txt"))

#save pairwise test of fitted model (estimated marginal means) as .txt file
capture.output(pairs(emmeans(lm.LRR, ~ treatment)), file = paste0(output3.f.fp, "/FXM_lmLRR_pairwise.txt"))
...
#save as .csv file
write.csv(df.LRR, file = paste0(output3.f.fp, '/FXM_dfLRR_summary_statistics.csv'))
```
...
#save as R file
saveRDS(df.LRR, paste0(objects3.f.fp, "/FXM_dfLRR.rds"))
...
```{r echo = FALSE, include = FALSE}
#Restore dfLRR
#df.LRR <- readRDS(objects3.f.fp, "FXM_dfLRR.rds")
...
i.plot.f <- df.LRR %>% 
  ...
  geom_hline(yintercept = 1.006, linetype="dashed", color = "blue", size = 1.2) +
...
``` {r echo = FALSE, fig.pos = 'h', fig.cap = "Interaction plot between N and P of response ratios of nutrient enrichments vs. controls in Fryxell basin soils. Red dashed line represents control and blue dashed line indicates an additive."}
#display graph
plot(i.plot.f)
```
...
#Save as jpg
jpeg(filename = "output/fryxell/FXM_response_ratio_interaction_plot.jpeg", 
     width = 800, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
i.plot.f.m <- df.LRR %>% 
  ...
  scale_fill_manual(values=c("burlywood4",
                             ...
                             "springgreen4")) + 
  geom_hline(yintercept = 0, linetype="dashed", color = "red", size = 1.2) +
  geom_hline(yintercept = 1.03, linetype="dashed", color = "blue", size = 1.2) +
  guides(color = "none", fill = "none") +
  theme_bw() +
  theme(plot.title = element_text(size = 40), 
        ...
        plot.margin = margin(l = 80, # Left margin
                             r = 10,
                             b = 5, # bottom margin 37, 15
                             t = 10)) 



#save as R file
saveRDS(i.plot.f.m, paste0(objects3.f.fp, "/FXM_lnRR_plot_figure3.rds"))
