...
subtitle: "  \n Top ASVs Bonney Basin - 16S SSU rRNA gene"
author: "Adam J. Solon"
date: "`r Sys.Date()`"
...
asvTable.BAC.fp <- paste0(input2.fp, "/ASVtable_16S_wTax_bonney.txt") 
...
asvTab.BAC.b.np.2 <- asvTab.BAC.b.np.1 %>% select(-BNM_11)  
...
########################################################
# create row sum column
asvTab.BAC.b.np.3 <- asvTab.BAC.b.np.2 %>% mutate(total = rowSums(.))

# re-order with greatest row sum 1st
asvTab.BAC.b.np.3 <- asvTab.BAC.b.np.3 %>% arrange(desc(total))
...
########################################################
# create row sum column
asvTab.BAC.b.np.4 <- asvTab.BAC.b.np.3 %>% filter(total!= 0)
...
########################################################
# calculate relative abundance of each cell by column value
asvTab.BAC.b.np.5 <- asvTab.BAC.b.np.4 %>% apply(2, function(x){x/sum(x)})
# multiply by 100 for percentage
asvTab.BAC.b.np.6 <- asvTab.BAC.b.np.5 %>% apply(2, function(x){x *100})
# classify as a data frame
asvTab.BAC.b.np.6 <- as.data.frame(asvTab.BAC.b.np.6)
...
########################################################
# move rownames of ASV table into 1st column as 'ASV_ID'
asvTab.BAC.b.np.6 <- tibble::rownames_to_column(asvTab.BAC.b.np.6, "ASV_ID")
# Left Join using inner_join function from dplyr
asvTab.BAC.b.np.6.T <- asvTab.BAC.b.np.6 %>% inner_join(taxTab, by= "ASV_ID")
...
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.b.5.T, paste0(objects2.b.fp, "/asvTab.BAC.b.b.5.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.b.5.T, file = paste0(output2.b.fp , "/BonneyBULK_16S_RelativeAbundance.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)

# CTRL samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.c.5.T, paste0(objects2.b.fp, "/asvTab.BAC.b.c.5.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.c.5.T, file = paste0(output2.b.fp , "/BonneyCTRL_16S_RelativeAbundance.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)

# N samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.n.5.T, paste0(objects2.b.fp, "/asvTab.BAC.b.n.5.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.n.5.T, file = paste0(output2.b.fp , "/BonneyN_16S_RelativeAbundance.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)

# P samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.p.5.T, paste0(objects2.b.fp, "/asvTab.BAC.b.p.5.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.p.5.T, file = paste0(output2.b.fp , "/BonneyP_16S_RelativeAbundance.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)

# NP samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.np.6.T, paste0(objects2.b.fp, "/asvTab.BAC.b.np.6.T.rds"))
#save as .txt file
write.table(asvTab.BAC.b.np.6.T, file = paste0(output2.b.fp , "/BonneyNP_16S_RelativeAbundance.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)
...
#NP
asvTab.BAC.b.np.top <- asvTab.BAC.b.np.6.T %>% filter(total > 1.0)
...
knitr::kable(asvTab.BAC.b.b.top, booktabs = T, digits = 2, caption = 'Most Relatively Abundant ASVs in Bonney basin bulk samples') %>%
...
#create table of library  
knitr::kable(asvTab.BAC.b.c.top, booktabs = T, digits = 2, caption = 'Most Relatively Abundant ASVs in Bonney basin control samples') %>%
  kable_styling(latex_options = c("striped", "scale_down", "hold_position"), font_size = 12) %>%
  row_spec(0, bold = T)
...
#create table of library  
knitr::kable(asvTab.BAC.b.n.top, booktabs = T, digits = 2, caption = 'Most Relatively Abundant ASVs in Bonney basin N samples') %>%
  kable_styling(latex_options = c("striped", "scale_down", "hold_position"), font_size = 12) %>%
  row_spec(0, bold = T)
...
#create table of library  
knitr::kable(asvTab.BAC.b.p.top, booktabs = T, digits = 2, caption = 'Most Relatively Abundant ASVs in Bonney basin P samples') %>%
  kable_styling(latex_options = c("striped", "scale_down", "hold_position"), font_size = 12) %>%
  row_spec(0, bold = T)
...
#create table of library  
knitr::kable(asvTab.BAC.b.np.top, booktabs = T, digits = 2, caption = 'Most Relatively Abundant ASVs in Bonney basin NP samples') %>%
  kable_styling(latex_options = c("striped", "scale_down", "hold_position"), font_size = 12) %>%
  row_spec(0, bold = T)
...
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.b.top, paste0(objects2.b.fp, "/asvTab.BAC.b.b.top.rds"))
#save as .txt file
write.table(asvTab.BAC.b.b.top, file = paste0(output2.b.fp , "/BonneyBULK_16S_Top.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)

# CTRL samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.c.top, paste0(objects2.b.fp, "/asvTab.BAC.b.c.top.rds"))
#save as .txt file
write.table(asvTab.BAC.b.c.top, file = paste0(output2.b.fp , "/BonneyCTRL_16S_Top.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)

# N samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.n.top, paste0(objects2.b.fp, "/asvTab.BAC.b.n.top.rds"))
#save as .txt file
write.table(asvTab.BAC.b.n.top, file = paste0(output2.b.fp , "/BonneyN_16S_Top.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)

# P samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.p.top, paste0(objects2.b.fp, "/asvTab.BAC.b.p.top.rds"))
#save as .txt file
write.table(asvTab.BAC.b.p.top, file = paste0(output2.b.fp , "/BonneyP_16S_Top.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)

# NP samples
########################################################
# save as an R file
saveRDS(asvTab.BAC.b.np.top, paste0(objects2.b.fp, "/asvTab.BAC.b.np.top.rds"))
#save as .txt file
write.table(asvTab.BAC.b.np.top, file = paste0(output2.b.fp , "/BonneyNP_16S_Top.csv"), 
            sep = ",", row.names = TRUE, col.names = NA)
